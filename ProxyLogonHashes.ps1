...
$Paths = @("c:\inetpub","c:\Program Files\Microsoft\Exchange Server")
...
$MD5 = @("4b3039cf227c611c45d2242d1228a121","0fd9bffa49c76ee12e51e3b8ae0609ac","79eb217578bed4c250803bd573b10151")

ForEach ($Path in $Paths) {
    $Items = Get-ChildItem -Path "$($Path)\*.aspx" -Recurse -Force | Select FullName
    ForEach ($Item in $Items){
        $Hash = Get-FileHash $Item.FullName -Algorithm SHA256
        If ($Sha256 -contains $Hash.Hash){
            Write-Host $Item.FullName -ForegroundColor Red
            $Found = $Found + 1
        }
		$Hash = Get-FileHash $Item.FullName -Algorithm MD5
        If ($MD5 -contains $Hash.Hash){
            Write-Host $Item.FullName -ForegroundColor Red
            $Found = $Found + 1
        }
    }
...
if ($Found -eq 0){
    ...
    Write-Host "Total" $Found " suspicious file(s) detected" -ForegroundColor Red
}
